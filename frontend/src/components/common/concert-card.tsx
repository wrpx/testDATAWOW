...
export function ConcertCard({ concert, actionLabel, actionTone, onAction }: ConcertCardProps) {
  return (
    <article className="rounded-lg border border-app-border bg-transparent p-5 sm:p-6 lg:p-8">
      ...
      <div className="mt-6 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-end">
        ...
        <button
          ...
          className={`min-w-32 rounded-md px-5 py-2.5 text-lg font-semibold text-white ${actionToneMap[actionTone]}`}
        >
          {actionLabel}
        </button>
      </div>
